...
function App() {
  ...
  return (
    <div className="app">
      {User ? Admin=== User?._id && <div className='float'><Float/></div> : <></>}
      <Topbar/>
      <Routes>
        ...
        <Route path='/settings' element={User!==null ? <Settings/> : <Login/>}/>
        <Route path='/auth' element={User!==null ? <Home/> : <Login/>}/>
        {User ? Admin=== User._id &&<Route path='/users' element={<UserPage/>}/> : <></>}
      </Routes>
      {User && <Footer/>}
    </div>
  )
